...
class VerificationCodeService {
  ...
  public function isVerified($user_id): bool {
    $record = $this->connection
      ->query("SELECT user_id FROM {$this->table} WHERE user_id = :user_id AND verified = 1",
        [':user_id' => $user_id])
      ->fetchField();
    if ($record) {
      return TRUE;
  ...
  public function checkOtpCode($code): bool {
    $record = $this->connection->query("SELECT user_id FROM {$this->table} WHERE code = :code",
        [
          ':code' => $code,
        ]
      )
  ...
  public function verify($code) {
    ...
    $user_id = $this->connection
      ->query("SELECT user_id FROM {$this->table} WHERE code = :code", [':code' => $code])
      ->fetchField();
